...
{
    int val = strcmp( (char *) s1, (char *) s2 );
    return val == 0;
}
...
Value *list_lookup(Node *list, Hashable *key)
{
    Node *cur_node = list;
    while (cur_node != NULL) {
        if (equal_hashable(cur_node->key, key))
...
Map *make_map(int n)
{
    ...
    mp->n = n;
    mp->lists = (Node **) malloc(sizeof(Node *) * n);
    for (int i = 0; i < n; i++) {
        mp->lists[i] = NULL;
    }
    return mp;
...
{
    int hash_val = hash_hashable(key) % (map->n);
    Node *map_list = map->lists[hash_val];
    map->lists[hash_val] = prepend(key, value, map_list);
}
...
{
    int hash_val = hash_hashable(key) % (map->n);
    Value *val = list_lookup(map->lists[hash_val], key);
    return val;
}
